...
# Toggle iPoRai4 as extended display via Sidecar + displayplacer; no arg = toggle; ipadx [on|off]
ipadx() {
  local _IPAD_ID="AC0E2873-B364-C6B3-C90B-A2661D91F570"
  local _MAC_ID="37D8832A-2D66-02CA-B9F7-8F30A301B230"
  local _connected
  displayplacer list 2>/dev/null | grep -q "$_IPAD_ID" && _connected=1 || _connected=0

  local _action="${1:-toggle}"
  [ "$_action" = "toggle" ] && { [ "$_connected" -eq 1 ] && _action=off || _action=on; }

  case "$_action" in
    on)
      [ "$_connected" -eq 1 ] && { echo "iPoRai4 already connected"; return 0; }
      osascript << 'AS' || { echo "AppleScript failed — connect manually: Control Center → Screen Mirroring → iPoRai4"; return 1; }
tell application "System Events"
  tell process "ControlCenter"
    click menu bar item 6 of menu bar 1
    delay 0.8
    set _smClicked to false
    repeat with _el in (entire contents of (window 1))
      try
        if (class of _el is button) and (description of _el contains "Screen Mirroring") then
          click _el; set _smClicked to true; exit repeat
        end if
      end try
    end repeat
    if not _smClicked then key code 53; error "Screen Mirroring not found in Control Center" end if
    delay 0.8
    set _ipClicked to false
    repeat with _el in (entire contents of (window 1))
      try
        if (class of _el is button) and (description of _el contains "iPoRai4") then
          click _el; set _ipClicked to true; exit repeat
        end if
      end try
    end repeat
    delay 0.3; key code 53
    if not _ipClicked then error "iPoRai4 not found — ensure iPad is nearby and Sidecar is available" end if
  end tell
end tell
AS
      echo "Connecting… waiting 4s for Sidecar"
      sleep 4
      displayplacer \
        "id:$_MAC_ID res:1680x1050 hz:60 color_depth:8 enabled:true scaling:on origin:(0,0) degree:0" \
        "id:$_IPAD_ID res:1536x1200 hz:60 color_depth:4 enabled:true scaling:on origin:(-1536,-106) degree:0" \
        && echo "iPoRai4 extended left" || echo "Arrange failed — iPad may still be connecting; retry: ipadx on"
      ;;
    off)
      [ "$_connected" -eq 0 ] && { echo "iPoRai4 not connected"; return 0; }
      osascript << 'AS' || { echo "AppleScript failed — disconnect manually: Control Center → Screen Mirroring → iPoRai4"; return 1; }
tell application "System Events"
  tell process "ControlCenter"
    click menu bar item 6 of menu bar 1
    delay 0.8
    ...
    delay 0.3; key code 53
    if not _ipClicked then error "iPoRai4 not found in Screen Mirroring list" end if
  end tell
end tell
AS
      echo "iPoRai4 disconnected"
      ;;
    *)
      echo "Usage: ipadx [on|off]  (no arg = toggle)"
      ;;
  esac
}
