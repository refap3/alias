...
```
  ...
  2) deb               -     /Users/rainers/deb  (linux only)
...
pc  p   pcp              pcpv               pcpa             pcp203cv  pcs203cv
    c   pcc              pccv               pcca
    w   pcw              pcwv               pcwa
    wl  pcwl             pcwlv              pcwla
    wg  pcwg             pcwgv              pcwga
...
| `raphacv` | SSH → HA Vienna via Cloudflare tunnel (`ssh32.deprec.uk`) port 22222 |
| `raphaca` | SSH → HA Aigen via Cloudflare tunnel (`ssh5.cfaig2vie.uk`) port 22222 |
...
| `pcs203cv` | SSH → `ssh203.deprec.uk` (PC .203 Vienna) via Cloudflare tunnel, plain shell |
...
Reports are written to `~/.dotfiles/clinternal_sessions/sessions_YYYY-MM-DD_HHMM.{md,html}`.

The launcher opens whatever path the tool prints on its `HTML:` line, so the output
directory is defined only in `extract_sessions.py` — the alias never hardcodes it.

The HTML report includes per-session expand/collapse panels, a date-range filter, text search, and cost tracking.

**AI summaries** — each session gets a one-line description (summary bar) and a 3–5 sentence paragraph (expanded panel), generated by `claude-haiku-4-5-20251001` and cached in `~/.dotfiles/clinternal_sessions/summaries_cache.json`.

To enable summaries, save your Anthropic API key:
```bash
echo "sk-ant-..." > ~/.config/anthropic/api_key
chmod 600 ~/.config/anthropic/api_key
```
Or export `ANTHROPIC_API_KEY` in your environment. Pass `--no-summaries` to skip AI calls entirely:
```bash
csess --no-summaries
...
Reports are written to `~/.dotfiles/clinternal_memory/config_YYYY-MM-DD_HHMM.{md,html}` by
`~/.dotfiles/tools/extract_config.py`. No API calls, no cost. As with `csess`, the launcher
opens the path the tool prints on its `HTML:` line rather than globbing a hardcoded directory.
...
Chapter headings, quotations, names, `alt` text and navigation documents are never summarised, so the table of contents keeps working — headings are found by what the TOC points at, not by assuming they are `<h1>` tags, because many books style them as ordinary paragraphs. Anything finer — tone, what to protect, what to cut first — goes in `--context` as a sentence, not a flag. Summaries are cached separately per setting, so re-running at the same one is free — but a summary run never reuses a translation run's prose, since a translation is not a compaction. Only headings, `alt` text and nav labels are shared, and only when both runs translate into the same language and are pointed at the same `--cache` file. A German summary is cheapest and reads best as one pass: `epubtrans book.epub --summarize deep -l de`. Check the result with `verify.py book.epub book.summ-deep.epub --summary --same-language`, which reports the compaction achieved and fails if any link target was lost.
...
| `div [-s\|-c] [-r\|-x] [--sort KEY] [name]` | Live docker TUI: container overview + resource usage; keyboard filters/sort/dig/logs/manipulate(start/stop/restart) |
