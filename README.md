...
| `piv [hosts_file]` | Launch the terminal ping monitor (`~/deb/pinginfoview`). Auto-detects site via DNS (`ssb8.local`→Vienna, `pi.hole`→Aigen); falls back to `hosts.txt`. Pass an explicit path to override. Prints install instructions if deb repo is missing |
