...
Reports are written to `~/.dotfiles/sessions/sessions_YYYY-MM-DD_HHMM.{md,html}`.

The HTML report includes per-session expand/collapse panels, a date-range filter, text search, and cost tracking.

**AI summaries** — each session gets a one-line description (summary bar) and a 3–5 sentence paragraph (expanded panel), generated by `claude-haiku-4-5-20251001` and cached in `~/.dotfiles/sessions/summaries_cache.json`.
